...
class QuestsSearchController {
    ...
    func search(request: Sh_Generated_SearchQuestsRequest, completion: @escaping (_ result: Result) -> Void) {
        ...
        do {
            let _ = try Sh_Generated_QuestsServiceServiceClient(address: ApiConfig().address).search(request) { response, callResult in
    ...
    private func fakeResponse(request: Sh_Generated_SearchQuestsRequest, completion: @escaping (_ result: Result) -> Void) {
        ...
        quest2.order.description_p = "Если ты устал на работе и хочегь покидать говно в вальере, то это преддложение для тебя. Лопату предоставим."
